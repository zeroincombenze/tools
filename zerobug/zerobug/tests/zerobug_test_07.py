...
class RegressionTest():
    ...
    def get_ocb_tree(self, ver, name=None, hy=None):
        ...
        if ver in ('7.0', '8.0'):
            OS_TREE = (ocb_name,
                       '%s/addons' % ocb_name,
                       '%s/server/openerp' % ocb_name if hy
                       else '%s/openerp' % ocb_name,
                       '%s/.git' % ocb_name,
                       )
            ...
            OS_TREE = (ocb_name,
                       '%s/addons' % ocb_name,
                       '%s/odoo/odoo' % ocb_name if hy
                       else '%s/odoo' % ocb_name,
                       '%s/.git' % ocb_name,
                       )
    ...
    def do_test_ocb(self, z0ctx, ver, name=None, hy=None):
        ...
        if not z0ctx['dry_run']:
            self.root = z0testodoo.build_odoo_env(
                z0ctx, ver, name=name, hierarchy=hy)
        for path in OS_TREE:
            if not z0ctx['dry_run']:
    ...
    def do_test_repo(self, z0ctx, ver, name=None, hy=None):
        ...
        if not z0ctx['dry_run']:
            repodir = z0testodoo.create_repo(
                z0ctx, self.root, 'test_repo', ver, hierarchy=hy, name=name,
                repotype=repotype)
    ...
    def test_01(self, z0ctx):
        ...
        for ver in VERSIONS:
            ...
            sts += self.do_test_ocb(
                z0ctx, ver, name=name)
            sts += self.do_test_repo(
                z0ctx, ver, name=name)
    ...
    def test_05(self, z0ctx):
        ...
        if not z0ctx['dry_run']:
            ...
            self.root = z0testodoo.build_odoo_env(
                z0ctx, branch, name=name)
            odoo_path = os.path.join(
                os.path.expanduser('~/'), 'OCB-%s' % branch)
        ...
        if not z0ctx['dry_run']:
            ...
            z0testodoo.git_clone(remote, reponame, branch, odoo_path)
        sts += self.Z.test_result(z0ctx,
                                  'git clone OCA/%s/14.0' % reponame,
                                  True,
                                  os.path.isdir(odoo_path))
        return sts

    def setup(self, z0ctx):
        root = os.path.join(
            os.path.expanduser('~/'), 'OCB-14.0')
        self.os_tree = [root,
