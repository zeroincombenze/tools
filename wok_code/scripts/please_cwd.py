...
class PleaseCwd(object):
    ...
    def get_config(self):
        ...
        if not rpcport:
            rpcport = build_odoo_param("RPCPORT", odoo_vid=".", multi=True)
    ...
    def assure_doc_dirs_odoo(self, is_repo=False):
        ...
        if (
            ...
            and not is_repo
            and (
                not pth.isfile(pth.join(docdir, "CONTRIBUTORS.rst"))
    ...
    def assure_doc_dirs(self, docdir=None, pkgtype=None, is_repo=False):
        ...
        if pkgtype == "pypi":
            ...
            if not pth.isfile(logo):
                srclogo = pth.join(
                    please.get_pkg_tool_dir(pkgtool=True), "docs", "logozero_180x46.png"
                )
                if please.opt_args.verbose:
                    print(
                        "%s cp %s %s"
                        % (">" if please.opt_args.dry_run else "$", srclogo, logo)
                    )
                if not please.opt_args.dry_run:
    ...
    def build_po_list(self, path, repo, excl=None):
        po_list = []
        fqn = pth.join(path, repo)
        if pth.isdir(fqn):
            for root, dirs, files in os.walk(fqn):
                if root == excl:
                    continue
                for fn in files:
                    if fn == "it.po":
    ...
    def do_clean_db(self):
        ...
        for row in cr.fetchall():
            ...
            if datetime.strftime(db_date, "%Y-%m-%d") < date_limit and rex.match(
                db_name
            ):
                sts = please.os_system(
                    "dropdb -U%s %s" % (db_user, db_name), rtime=True
                )
                if sts:
                    break
                ...
                for line in fd.read().split("\n"):
                    if "entry: arcangelo" in line:
                        if branch:
                            ...
                            target += re.sub(
                                "entry: arcangelo .*", "entry: arcangelo -i -P\n", line
                            )
                        continue
                    ...
                    if trig == "odoo":
                        ...
                        if not branch and line.startswith(" "):
                            ...
                            if ".egg-info/" not in line and pth.isdir(
                                pth.join(
        ...
        if please.is_odoo_pkg():
            ...
            if sts == 0:
                ...
                if (
                    ...
                    found_ocb = True
                    cur_repo = ""
                    while not please.is_repo_ocb(path=ocb_path):
                        if ocb_path == os.environ["HOME"]:
                            found_ocb = False
                            break
                        if not cur_repo and please.is_repo_odoo(path=ocb_path):
                            cur_repo = pth.basename(ocb_path)
                        ocb_path = pth.dirname(ocb_path)
                    po_list = []
                    if found_ocb:
                        for repo in REF_REPO:
                            po_list += self.build_po_list(
                                ocb_path, repo, excl=os.getcwd()
                            )
                        if cur_repo:
                            po_list += self.build_po_list(
                    ...
                    if po_list:
                        ...
                        cmd = "msgcat --use-first " + " ".join(po_list)
                        cmd += " | msgattrib --translated -o %s" % compendium_fqn
        ...
        if please.is_pypi_pkg():
            ...
            if pkgname != "tools":
                ...
                for fn in sorted(os.listdir(vme_dir)):
                    ...
                    if not pth.isdir(tgtdir) or not pth.isdir(pth.join(tgtdir, "bin")):
                        continue
                    sts = please.os_system(
                        "vem %s update %s" % (tgtdir, pth.dirname(srcdir)),
                        rtime=True,
    ...
    def do_version(self):
        ...
        if please.opt_args.from_version:
            ...
            REGEX_DICT_VER = re.compile(
                "^ *([\"']version[\"']: [\"'])(%s)[\"']" % please.opt_args.from_version
            )
            ...
            REGEX_TESTENV_VER = re.compile(r"^(.*v)(\d+\.\d+\.\d+)")
        if please.is_pypi_pkg():
            sts = 0
            self.ref_version = ""
            for root, dirs, files in os.walk(self.cur_path_of_pkg()):
