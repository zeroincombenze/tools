...
TEST_REP_OCB = {
    ...
    % (ROOT_URL, TEST_REP),
    'collaborators_url': '%s%s/collaborators{collaborator}' % (ROOT_URL, TEST_REP),
    'updated_at': '2013-07-15T20:26:20Z',
    'private': False,
    'pulls_url': '%s%s/pulls{/number}' % (ROOT_URL, TEST_REP),
    'disabled': False,
    'issue_comment_url': '%s%s/issues/comments{number}' % (ROOT_URL, TEST_REP),
    'labels_url': '%s%s/labels{/name}' % (ROOT_URL, TEST_REP),
    'has_wiki': True,
    'full_name': 'zeroincombenze/%s' % TEST_REP,
    'owner': {
        'following_url': '%s%s/following{other_user}' % (ROOT_URL, TEST_REP),
        'events_url': '%s%s/events{/privacy}' % (ROOT_URL, TEST_REP),
        ...
        'subscriptions_url': '%s%s/subscriptions' % (USER_URL, TEST_REP),
        'node_id': 'MDQ6VXNlcjY5NzI1NTU=',
        'test_repos_url': '%s%s/test_repos' % (USER_URL, TEST_REP),
        'received_events_url': '%s%s/received_events' % (USER_URL, TEST_REP),
        'gravatar_id': '',
        'starred_url': '%s%s/starred{/owner}{/TEST_REPo}' % (USER_URL, TEST_REP),
        'site_admin': False,
        'login': 'zeroincombenze',
        'type': 'User',
        'id': 6972533,
        'followers_url': '%s%s/followers' % (USER_URL, TEST_REP),
        'organizations_url': '%s%s/orgs' % (USER_URL, TEST_REP),
    },
    'statuses_url': '%s%s/statuses/{sha}' % (ROOT_URL, TEST_REP),
    'id': 58389479,
    'keys_url': '%s%s/keys{/key_id}' % (ROOT_URL, TEST_REP),
    ...
    'contents_url': '%s%s/contents/{+path}' % (ROOT_URL, TEST_REP),
    'has_pages': False,
    ...
    'milestones_url': '%s%s/milestones{/number}' % (ROOT_URL, TEST_REP),
    'languages_url': '%s%s/languages' % (ROOT_URL, TEST_REP),
    'size': 1884251,
    'homepage': '',
    'fork': True,
    'commits_url': '%s%s/commits{/sha}' % (ROOT_URL, TEST_REP),
    'releases_url': '%s%s/releases{id}' % (ROOT_URL, TEST_REP),
    'issue_events_url': '%s%s/issues/events{/number}' % (ROOT_URL, TEST_REP),
    'archive_url': '%s%s/{archive_format}{/ref}' % (ROOT_URL, TEST_REP),
    ...
    'forks': 0,
    'compare_url': '%s%s/compare/{base}...{ead}' % (ROOT_URL, TEST_REP),
    'open_issues': 0,
    'node_id': 'MDEwOlJlcG9zaXRvcnk1ODM4OTkzNg==',
    'git_url': 'git://github.com/zeroincombenze/%s.git' % TEST_REP,
    ...
    'git_commits_url': '%s%s/git/commits{/sha}' % (ROOT_URL, TEST_REP),
    ...
    while 1:
        ...
        if ctx['dry_run']:
            ...
            try:
                ...
                if (
                    not done_default
                    and git_org == 'zeroincombenze'
                    and ctx['odoo_vid'] in ('7.0', '8.0')
                ):
                    done_default = True
                    data = [
                        {'url': '//account-closing'},
