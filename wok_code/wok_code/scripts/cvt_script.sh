...
# (C) 2016-25 by SHS-AV s.r.l. - http://www.shs-av.com - info@shs-av.com
#
READLINK=$(which greadlink 2>/dev/null) || READLINK=$(which readlink 2>/dev/null)
export READLINK
# Based on template 2.0.19
THIS=$(basename "$0")
...
READLINK=\$(which greadlink 2>/dev/null) || READLINK=\$(which readlink 2>/dev/null)
export READLINK
# Based on template $(echo $__version__ | grep --color=never -Eo "[0-9]+\.[0-9]+\.[0-9]+" | head -n1)
THIS=\$(basename "\$0")
TDIR=\$(readlink -f \$(dirname \$0))
[ \$BASH_VERSINFO -lt 4 ] && echo "This script \$0 requires bash 4.0+!" && exit 4
if [[ -z \$HOME_DEVEL || ! -d \$HOME_DEVEL ]]; then
