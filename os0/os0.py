...
os0.py is platform independent and can run on Linux, OpenVMS and Linux.
...
if i >= 0:
    bg = bg[0:i]
bgout_fn = homedir+"/"+bg+"-{0:08x}".format(os.getpid())+".out"
bginp_fn = os.devnull
tlog_fn = ""

_xtl_dev_win = {'null:': 'nul'}
_xtl_dev_vms = {'null:': 'NL0:'}


def str2bool(t, dflt):
    if t.lower() in ["true", "t", "1", "y", "yes", "on", "enabled"]:
        return True
        ...
        if i >= 0:
            ...
            if (len(x) > 1
                    ...
                    and (os.path.exists(d)
                         or (_platform == "OpenVMS"
                         and os.path.exists(x[1])))):
                dev = x[1] + ":"
                filename = ""
...
def setlfn_win(filename, cnv_type):
    global LFN_FLAT, LFN_EXE, LFN_CMD, LFN_DIR

    filename, dev = extract_device(filename)
    if dev in _xtl_dev_win:
        dev = _xtl_dev_win[dev]
...
def setlfn_linux(filename, cnv_type):
    global LFN_FLAT, LFN_EXE, LFN_CMD, LFN_DIR

    return filename


def setlfn_vms(filename, cnv_type):
    global LFN_FLAT, LFN_EXE, LFN_CMD, LFN_DIR

    filename, dev = extract_device(filename)
    if dev in _xtl_dev_vms:
        dev = _xtl_dev_vms[dev]
...
def setlfilename(filename, cnv_type=LFN_FLAT):
    # Translate Linux filename into local OS
    if _platform == "win32":
        return (setlfn_win(filename, cnv_type))
    elif _platform == "OpenVMS":
        return (setlfn_vms(filename, cnv_type))
    return (setlfn_linux(filename, cnv_type))



def set_tlog_file(filename, new=False, dir4debug=None):
    ...
    tlog_fn = p + os.sep + fn
    if new:
        tlog_fd = open(tlog_fn, 'w')
        tlog_fd.close()


def set_debug_mode(dbg_mode=None):
...
def muteshell(cmd, simulate=False, tlog=False, keepout=False):
    # Execute command/batch file using OS shell and redirect output into file
    ...
    if _platform == "OpenVMS":
        ...
        if simulate or keepout:
            stdout_fd = open(bgout_fn, "w")
            s = bginp_mb.read()
            stdout_fd.write(s)
            stdout_fd.close()
    else:
        stdout_fd = open(bgout_fn, "w")
        stdinp_fd = open(bginp_fn, "r")
        if simulate:
            cmd = "echo " + cmd
...
if _platform == "OpenVMS":
    bginp_fn = setlfilename("/dev/null", LFN_FLAT)