        ...
        if not so.force_invoiced:
            for ln in so.order_line:
                ...
                if (
		         ...
		         and ( 
			 not so.carrier_id or so.carrier_id.product_id != ln.product_id)):
...
def log(mesg):
    print(mesg)
    with open(os.path.expanduser("~/close_sale_order.log"), "a") as fd:
        fd.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + mesg + "\n")
...
def main(cli_args=[]):
    ...
    rctr = wctr = 0
    for so in clodoo.browseL8(ctx,_so,clodoo.searchL8(ctx,_so,[
        ("state","in",["sale","done"]),
        ("invoice_status","not in",["no", "invoiced"]),
        ("delivery_state","=","done")
    ])):
        print("SO %s, sts=%s" % (so.name, so.invoice_status))
        rctr += 1
        astate="invoiced"
        for ln in so.order_line:
            if ln.product_id and ln.product_id.code == "CONAI":
                continue
            if ln.product_uom_qty > ln.qty_invoiced:
                astate = so.invoice_status
                break
        if astate != so.invoice_status:
            log("**** Invalid state %s for SO %s: set to %s!"
                    % (so.invoice_status, so.name, astate))
            clodoo.executeL8(ctx,_so,"action_done",[so.id])
            clodoo.writeL8(ctx,_so,[so.id],{"force_invoiced": True})
            wctr += 1
    log("%d records read, %d records written" % (rctr, wctr))
