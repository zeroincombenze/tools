...
def test_synchro_vg7(ctx):
    ...
    def general_check(ctx, model, id, vals):
        ...
        if model == 'res.partner' and vals.get('vg7_id') == 17:
        ...
        for ext_ref in vals:
            ...
            if ext_ref in ('vg7:shipping', 'vg7:billing', 'vg7:name'):
                continue
            elif model == 'account.payment.term' and ext_ref == 'vg7:code':
                continue
            elif (model == 'res.partner' and
                  ext_ref == 'vg7:customer_shipping_id'):
                vals[ext_ref] += 100000000
                loc_name = 'vg7_id'
    ...
    def init_test(ctx, company_id):
        ...
        clodoo.browseL8(
            ctx, model, env_ref(
                ctx, 'base.res_partner_1'), {'name': 'ASUSTek'})
        clodoo.browseL8(
            ctx, model, env_ref(
                ctx, 'base.res_partner_2'), {'name': 'Agrolait'})
        clodoo.browseL8(
            ctx, model, env_ref(
                ctx, 'base.res_partner_3'), {'name': 'China Export'})
        clodoo.browseL8(
            ctx, model, env_ref(
                ctx, 'base.res_partner_4'), {'name': 'Delta PC'})
    ...
    vals['vg7_id'] = vals['customer_shipping_id'] + 100000000
    del vals['customer_shipping_id']
    check_partner(
        ctx, get_id_from_vg7id(
            ctx, 'res.partner', vg7_id + 100000000), jacket_vals(vals))

    print('*** Starting odoo 2 odoo test ***')
...
def simulate_vg7(ctx):
    ...
    vals = {
        'id': 44,
        'billing_city': 'Imola',
        'billing_codice_univoco': '5RUO82D',
        'billing_company': 'La Romagnola S.A.S. di Marangoni Aldo',
        ...
        'billing_street': 'Via Brodolini',
        'billing_street_number': '8',
        'billing_country': 'Italia',
        'billing_country_id': 39,
        'billing_email': 'melania@laromagnola.it',
        'billing_piva': 'IT01598041208',
        'billing_cf': '02151140361',
    }
    write_file_2_pull(ext_model, vals)
    ext_model = 'customers_shipping_addresses'
    vals = {
        'customer_shipping_id': 17906,
        'customer_id': 44,
        'shipping_city': 'Imola',
        'shipping_company': 'La Romagnola S.A.S. di Marangoni Aldo',
        'shipping_postal_code': '40026',
        'shipping_region': 'BOLOGNA',
        'shipping_region_id': 54,
        'shipping_street': 'Piazza Maggiore',
        'shipping_street_number': '18',
    ...
    vals = {
        'vg7_id': 44,
        'city': 'Imola',
        'codice_destinatario': '5RUO82D',
        'name': 'La Romagnola S.A.S. di Marangoni Aldo',
        'zip': '40026',
        'mobile': '',
        'electronic_invoice_subjected': True,
        'vg7:region': 'BOLOGNA',
        'id': 17906,
        'phone': '0542 640502',
        'street': 'Via Brodolini, 8',
        'is_company': True,
        'vg7:country': 'Italia',
        'customer': True,
        'email': 'melania@laromagnola.it',
        'vat': 'IT01598041208',
    ...
    vals = {
        ...
        'vg7_id': 2187,
        'vg7_partner_id': 44,
        'vg7_partner_shipping_id': -1,
        'state': 'sale',
        'partner_shipping_id': 17907,
        'vg7:courier_id': 0,
        'partner_id': 17906,
        'vg7_agent_id': 8,
    }
...
def solve_unnamed(ctx):
    ...
    for rec in clodoo.browseL8(
        ...
        clodoo.writeL8(ctx, model, rec.id, {'name': rec.name + ' '})
        clodoo.writeL8(ctx, model, rec.id, {'name': rec.name})
        ctr += 1
    print('%d record updated' % ctr)
...
print(' PURCHASE ORDER                  - update_einvoice_out_attachment')
print(' - close_purchase_orders         - unlink_einvoice_out_attachment')
print(' PRODUCT                         - set_tax_code_on_invoice')
print(' - set_products_2_consumable     - set_comment_on_invoice')
print(' - set_products_delivery_policy')
print(' ACCOUNT                         DELIVERY/SHIPPING')
print(' - create_RA_config              - change_ddt_number')
print(' COMMISSION                      - create_delivery_env')
print(' - create_commission_env         - show_empty_ddt')
print(' PARTNER/USER                    RIBA')
print(' - configure_fiscal_position     - configure_RiBA')
print(' - set_ppf_on_partner            - manage_riba')
print(' - deduplicate_partner')
