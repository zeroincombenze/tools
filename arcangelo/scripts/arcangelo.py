    ...
    def load_config(self, confname, ignore_not_found=True, prio=1, no_store=False):
        ...
        for (name, item) in rules.items():
            ...
            if not isinstance(item, dict):
                self.raise_error("Invalid rule <%s> in configuration file %s!"
                                 % (name, configpath))
            if "ctx" in item:
                if not eval(item["ctx"], self.__dict__):
                    rules_2_rm.append(name)
                    continue
            if "include" in item:
                if item["include"] not in self.rule_categ:
                    if "pass1" in item.get("ctx", ""):
                        rules_2_load_pass1.append(item["include"])
                    else:
                        rules_2_load.append(item["include"])
                rules_2_rm.append(name)
                continue

            if "match" not in item and "search" not in item and "expr" not in item:
                self.raise_error(
        ...
        for (name, item) in rules.items():
            ...
            if (
                ...
                and item["prio"] > self.mig_rules[name]["prio"]
            ):
            ...
            if (
                    ...
                    and item["prio"] > self.pass1_rules[name]["prio"]
            ):
                # Less priority rule than current
                rules_2_rm.append(name)
        ...
        if action == "mv":
            ...
            if len(args) > 1:
                if re.match("[+-][0-9]+", args[1]):
                    ...
                    self.set_trigger(args[0], args[1])
            else:
    ...
    def update_line(self, subrule, regex, lineno, key=None):
        ...
        if action == "s":
            ...
            if not args:
                self.raise_error("Action set trigger (+) w/o trigger name!")
                return False, 0
            x = re.match(self.mig_rules[key]["match"],
                         self.lines[lineno]) if "match" in self.mig_rules[key] else None
            if x and x.groups():
                ...
                if self.lines[lineno]:
                    if not re.match("^ *#", self.lines[lineno]):
                        ...
                        if (
                            ...
                            self.transition_stage = (
                                self.stage if self.stage != "header" else "import")
