...
def main(cli_args=None):
    if not cli_args:
        cli_args = sys.argv[1:]
    if len(cli_args) < 2:
        print("usage: %s srcpath tgtpath|-a" % sys.argv[0])
        return 3
    srcpath = cli_args[0]
    tgtpath = cli_args[1]
    ...
    if not pth.isfile(srcpath):
        ...
        return 3
    if tgtpath == "-a":
        root = "/home/odoo/devel/pypi"
        for path in os.listdir(root):
            if not os.path.isdir(pth.join(root, path)):
                continue
            pkg = os.path.basename(path)
            tgtpath = pth.join(root, path, pkg, "scripts", "main.py")
            if not pth.isfile(tgtpath) or srcpath == tgtpath:
                continue
            print("Merge %s %s" % (srcpath, tgtpath))
            sts = merge(srcpath, tgtpath)
            if sts:
                break
    else:
