...
class Test:
    def __init__(self, zarlib):
        self.Z = zarlib
        self.cmd = (
            ["python"] + [self.Z.rundir + "/clodoo.py"] + ["-q"]
        )  # + [ '-c' + self.Z.testdir + '/clodoo.conf' ]
...
actions=unit_test
"""
        fd = open(self.Z.rundir + "/clodoo.conf", "w")
        fd.write(codefile)
        fd.close()
    ...
    def check_4_module(self, oe_version):
        ...
        confn = "%s/test_clodoo2.conf" % self.Z.testdir
        codefile = """[options]
xmlrpc_port=%d
oe_version=%s
""" % (
        ...
        fd.write(codefile)
        fd.close()
        uid, ctx = clodoo.oerp_set_env(confn=confn, db=dbname)
        model = "ir.module.module"
        ids = clodoo.searchL8(
            ctx, model, [("name", "=", self.module_2_test), ("state", "=", "installed")]
        )
        if ids:
            return True
        else:
    ...
    def check_4_user(self, oe_version, id=None, login_name=False):
        ...
        if not id:
            ...
            res, err = p.communicate()
            id = eval(res) or 1
        sql = "select login from res_users" + " where id=%d;" % id
        cmd = 'psql -At -U%s %s -c"%s"' % (user, dbname, sql)
        p = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
        res, err = p.communicate()
    ...
    def check_4_partner(self, oe_version, id=None, name=False, byname=False):
        ...
        name = name or "admin"
        if name and byname:
            sql = "select name from res_partner" + " where name='%s';" % name
        else:
            sql = "select name from res_partner" + " where id=%d;" % id
    ...
    def test_01(self, z0ctx):
        ...
        if not ctx["dry_run"]:
            ...
            res = req_version
        sts = self.Z.test_result(z0ctx, "Version", req_version, res)
        return sts
        ...
        if os.environ.get("HOSTNAME", "") in ("shsdef16", "shs17fid"):
            for oe_version in VERSIONS_TO_TEST:
                ...
                sts = self.Z.test_result(
                    z0ctx, "Show params -b%s" % oe_version, True, res
                )
        return sts
            ...
            for oe_version in VERSIONS_TO_TEST:
                if not ctx["dry_run"]:
                    ...
                    if os.environ.get("TRAVIS", "") != "true":
                        xmlrpc_port, dbname = self.param_by_db(oe_version)
                        os0.muteshell("/opt/odoo/tools/zar/pg_db_active -wa " + dbname)
                    ...
                    res = True
                sts = self.Z.test_result(
                    z0ctx, "Create DB -b%s" % oe_version, True, res
                )
                ...
                if not ctx["dry_run"]:
                    ...
                    if re.search("protocol *= *%s" % xml_prot, res):
                        ...
                        res = False
                    sts = self.Z.test_result(
                        z0ctx, "Show db params -b%s" % oe_version, True, res
                    )
        return sts

    def test_04(self, z0ctx):
        sts = TEST_SUCCESS
        confn = "%s/test_clodoo.conf" % self.Z.testdir
        # datafn = 'res_users.csv'
        # dataffn = '%s/%s' % (self.Z.testdir, datafn)
        if os.environ.get("HOSTNAME", "") in ("shsdef16", "shs17fid"):
            for oe_version in VERSIONS_TO_TEST:
                if not ctx["dry_run"]:
        ...
        if os.environ.get("HOSTNAME", "") in ("shsdef16", "shs17fid"):
            for oe_version in VERSIONS_TO_TEST:
                if not ctx["dry_run"]:
                    ...
                    res = False
                sts = self.Z.test_result(z0ctx, "User -b%s" % (oe_version), False, res)
                if not ctx["dry_run"]:
                    codefile = """[options]
actions=import_file
filename=%s
...
z0bug.user_admin2,Admin,admin2,"Amministratore2","me2@example.com",,%s
""" % (
                            self.login_2_test,
                            self.new_password,
                        )
...
z0bug.user_admin2,Admin,admin2,"Amministratore2","me2@example.com",,z0bug.partner_user2,%s
""" % (
                            self.login_2_test,
                            self.new_password,
                        )
                    ...
                    res = True
                sts = self.Z.test_result(
                    z0ctx, "Import user -b%s" % (oe_version), True, res
                )
                if not ctx["dry_run"]:
                    res = self.search_alias(oe_version, "z0bug.user_admin2")
                    res = self.check_4_user(oe_version, id=res, login_name="admin2")
                else:
                    res = True
                sts = self.Z.test_result(
                    z0ctx, "Add user -b%s" % (oe_version), True, res
                )
                if not ctx["dry_run"] and oe_version != "6.1":
                    res = self.search_alias(oe_version, "z0bug.partner_user2")
                    res = self.check_4_partner(oe_version, id=res, name="Admin")
                else:
                    res = True
                sts = self.Z.test_result(
                    z0ctx, "Add user -b%s" % (oe_version), True, res
...
""" % (
                    ...
                    cmd = self.bulk_cmd(oe_version)
                    cmd = cmd + ["-c%s" % confn]
                    datafile = """oe_versions,user,name,value
,admin2,base.group_system,True
"""
                    fd = open(dataffn, "w")
                    fd.write(datafile)
                    fd.close()
...
""" % crypt(
                    ...
                    res = True
                sts = self.Z.test_result(
                    z0ctx, "Connect user admin2 -b%s" % (oe_version), True, res
                )
        return sts
    ...
    def test_07(self, z0ctx):
        ...
        if os.environ.get("HOSTNAME", "") in ("shsdef16", "shs17fid"):
            for oe_version in VERSIONS_TO_TEST:
                if not ctx["dry_run"]:
                    res = self.check_4_partner(
                        oe_version, name=self.new_partner, byname=True
                    )
                else:
                    res = False
                sts = self.Z.test_result(
                    z0ctx, "Partner -b%s" % (oe_version), False, res
                )
                if not ctx["dry_run"]:
                    codefile = """[options]
login_user=admin2
...
"""
                    ...
                    res = True
                sts = self.Z.test_result(
                    z0ctx, "Import partner -b%s" % (oe_version), True, res
                )
        return sts
...
""" % (
                    ...
                    res = False
                sts = self.Z.test_result(z0ctx, "Drop DB -b%s" % oe_version, False, res)
        return sts
...
if __name__ == "__main__":
    Z = Z0test
    ctx = Z.parseoptest(sys.argv[1:], version=version())
