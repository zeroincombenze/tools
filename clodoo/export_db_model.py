...
def get_symbolic_value(ctx, model, name, value):
    ...
    if name:
        ...
        field_id = clodoo.searchL8(
            ctx, field_model, [("model", "=", model), ("name", "=", name)]
        )
        if field_id:
            relation = clodoo.browseL8(ctx, field_model, field_id[0]).relation
            name_id = clodoo.searchL8(
                ctx, name_model, [("model", "=", relation), ("res_id", "=", value)]
            )
            if name_id:
                model_rec = clodoo.browseL8(ctx, name_model, name_id[0])
                ...
                if name == "company_id":
                    ...
                    for sel_name in ("code", "name"):
                        ids = clodoo.searchL8(
                            ctx,
                            field_model,
                            [("model", "=", relation), ("name", "=", sel_name)],
                        )
                        if ids:
                            break
                    if ids:
                        value = clodoo.browseL8(ctx, relation, value)[sel_name]
                        value = "=${%s(%s)::%s}" % (relation, sel_name, value)
    else:
        name_id = clodoo.searchL8(
            ctx, name_model, [("model", "=", model), ("res_id", "=", value)]
        )
        if name_id:
            model_rec = clodoo.browseL8(ctx, name_model, name_id[0])
            value = "%s.%s" % (model_rec.module, model_rec.name)
        ...
        for nm in out_flds:
            ...
            if nm == "id":
                ...
                if isinstance(value, (int, long)):
                    data_valid = False
                    if not re.match(ctx["id_filter"], str(value)) or ctx["exclext"]:
                        discard = True
                        break
                else:
                ...
                if isinstance(rec[f[0]], (date, datetime)):
                    ...
                    if ttype == "date":
                        if value.year == current_year:
                            ...
                            value = "${_last_year}-%02d-%02d" % (value.month, value.day)
            elif f[1] == "id":
...
parser = z0lib.parseoptargs(
    "Export table from Odoo", "(C) 2017-2020 by SHS-AV s.r.l.", version=__version__
)
...
parser.add_argument(
    "-F", "--id-filter", help="match id", dest="id_filter", default=".*"
)
parser.add_argument(
    "-m",
    "--model",
...
parser.add_argument(
    "-o", "--out-file", help="output file", dest="out_file", metavar="file", default=""
)
parser.add_argument(
    "-S",
    "--search",
