...
class Test():
    ...
    def __test_05(self, z0ctx):
        ...
        for ver in VERSIONS_TO_TEST + ['v7', 'v8.0']:
            ...
            res = build_odoo_param('ROOT', odoo_vid=ver, suppl='crm',
                                   multi=True)
            sts = self.Z.test_result(z0ctx,
                                     'Root dir %s/crm' % ver,
                                     TRES[ver],
            ...
            res = build_odoo_param('HOME', odoo_vid=ver, suppl='OCB',
                                   multi=True)
            sts = self.Z.test_result(z0ctx,
                                     'Home dir %s/OCB' % ver,
                                     TRES[ver],
        ...
        for ver in VERSIONS_TO_TEST:
            w = 'VENV-%s' % ver
            res = build_odoo_param('ROOT', odoo_vid=w, suppl='crm',
                                   multi=True)
            sts = self.Z.test_result(z0ctx,
                                     'Root dir %s' % w,
                                     TRES[ver],
        ...
        for ver in VERSIONS_TO_TEST + ['v7', 'v8.0']:
            res = build_odoo_param('HOME', odoo_vid=ver, suppl='crm',
                                   multi=True)
            sts = self.Z.test_result(z0ctx,
                                     'Home %s/crm' % ver,
                                     TRES[ver],
