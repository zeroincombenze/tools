...
def build_alias_struct(mindroot, model, uname, type, fld_name=False):
    ...
    if (
        type == "value"
        and fld_name
        and (uname.find("^${") < 0 or not uname.endswith("}"))
    ):
...
def clean_transodoo(ctx=None):
    csv.register_dialect(
        "transodoo", delimiter=b"\t", quotechar=b'"', quoting=csv.QUOTE_MINIMAL
    )
    mindroot = {}
    with open(ctx["dict_fn"], "rb") as f:
        hdr = False
        reader = csv.DictReader(
            f, fieldnames=[], restkey="undef_name", dialect="transodoo"
        )
        for line in reader:
            row = line["undef_name"]
            if not hdr:
            ...
            if row[TYPE] == "merge":
                continue
            mindroot = build_alias_struct(mindroot, row[MODEL], row[NAME], row[TYPE])
            ver_names = []
            for ver in VERSIONS:
...
if __name__ == "__main__":
    parser = z0lib.parseoptargs(
        "Clean Transodoo", "© 2019 by SHS-AV s.r.l.", version=__version__
    )
    parser.add_argument("-h")
    parser.add_argument(
        "-f", "--tnl-file", action="store", dest="dict_fn", default="transodoo.csv"
    )
    ...
    elif os.path.isfile(os.path.join(os.path.expanduser("~"), "transodoo.csv")):
        ctx["dict_fn"] = os.path.join(os.path.expanduser("~"), "transodoo.csv")
    else:
